...
def has_hosted_usage() -> bool:
    ...
    for path in paths:
        try:
            ...
            try:
                if conn.execute(_PROBE_SQL).fetchone():
                    return True
            finally:
                conn.close()
        except Exception:
...
def scan(days_back: int = DEFAULT_DAYS_BACK, now: Optional[datetime] = None) -> Optional[ProviderUsageHistory]:
    ...
    for path in paths:
        ...
        try:
            conn = _open_readonly(path)
            try:
                raw = conn.execute(_DATA_SQL, (cutoff_ms,)).fetchone()
            finally:
                conn.close()
        except Exception:
            failures += 1
    ...
    series = [
        DailyUsageSeries(date=day, total_tokens=values["tokens"], estimated_cost=round(values["cost"], 2))
        for day, values in sorted(daily.items())
    ]
    model_usage = [
        ModelUsageSummary(model=name, total_tokens=values["tokens"], estimated_cost=round(values["cost"], 2))
